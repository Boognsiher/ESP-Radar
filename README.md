...
Open `ESP_Radar_v20.ino` in Arduino IDE
Configure `TFT_eSPI` for your display in its `User_Setup.h`
Edit the fallback WLAN credentials at the top of the `.ino`:
```cpp
   ...
   ```
Flash to your ESP32
Open the Web UI in a browser at the IP shown on the display
---
...
Create an account at opensky-network.org
Go to Account → API Client → Create new client
Copy the Client ID and Client Secret
Enter both in the Web UI under OpenSky
Save — the ESP will automatically fetch and refresh tokens
